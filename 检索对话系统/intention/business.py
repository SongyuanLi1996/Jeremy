...
# -*- coding: utf-8 -*-
'''
FilePath: /intention/business.py
...
import config
from utils.preprocessor import clean, filter_content, read_file
...
class Intention(object):
    ...
    def build_keyword(self, sku_path, to_file=None):
        """
        ...
        key_words = set()
        # chat.txt 的处理速度太慢，并且ware.txt中存在80000+名词，能够包揽绝大部分chat.txt中出现的名词，
        # 若只考虑使用ware.txt中的名词作为关键词，请令 self.include_train=False
        if self.include_train:
            logging.info('Extract key words from chat.txt.')
            tokens = self.data['custom'].dropna().progress_apply(lambda x: [
                token for token, pos in pseg.cut(x)
                if pos in ['n', 'vn', 'nz']
            ])
            key_words = set(
                [tk for sample in tokens for tk in sample if len(tk) > 1])
