...
# -*- coding: utf-8 -*-
"""
FilePath: /generative/bert_model.py
...
class BertLayer(nn.Module):
    ...
    def forward(self,
                ...
                attention_mask,
                output_attentions=False):
        attention_output, attention_matrices = self.attention(hidden_states,
                                                              attention_mask,
                                                              output_attentions=output_attentions)
        # FeedForward = self.output(self.intermediate(x),x)
        # = LN(dropout(w2(xw1+b1)+b2 + x))
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        if output_attentions:
            return encoder_layers, pooled_output, all_attention_matrices, (input_ids > 0).float()
        return encoder_layers, pooled_output, None, (input_ids > 0).float()
