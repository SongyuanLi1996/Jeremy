...
sys.path.append('..')
from config import batch_size, lr, root_path, bert_chinese_model_path, log_path, max_length, max_grad_norm, \
...
class Trainer:
    def __init__(self):
        ...
        logger.info('加载训练数据')
        train = SelfDataset(config.generative_train)
        logger.info('加载测试数据')
        dev = SelfDataset(config.generative_dev)
    ...
    def iteration(self, epoch, dataloader):
        ...
        for batch_idx, data in enumerate(tqdm(dataloader, position=0, leave=True)):
            ...
            predictions, loss, _, _, _ = self.bert_model(token_ids,
                                                      token_type_ids,
                                                      labels=target_ids
                                                      )
    ...
    def evaluate(self, epoch):
        ...
        with torch.no_grad():
            for token_ids, token_type_ids, target_ids in tqdm(self.devloader, position=0, leave=True):
                ...
                count += 1
        logger.info('evaluate epoch {} ,loss {}'.format(epoch, total_loss / count))
